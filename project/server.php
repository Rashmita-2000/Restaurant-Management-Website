...
if (isset($_POST['reg_user'])) {
  ...
  if (!(preg_match('/^[0-9]{10}+$/', $phno))){array_push($errors, "Enter valid phone number");}
  ...
  if (count($errors) == 0) {
  	...
  	$query = "INSERT INTO customer (username, email, password,address,phno) VALUES('$username', '$email', '$password','$address','$phno')";
  	mysqli_query($db, $query);
  	$_SESSION['customername'] = $username;
    
    header('location: home.php');
    echo '<script>alert("Successfully registered!!You are now logged in")</script>'; 
