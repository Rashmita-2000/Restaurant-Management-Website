...
<form action="" id="myForm" method="POST" class="form-container" style="display:block;">
  ...
  <script>
  function Pagedirect() {
   window.location.href = "dummyadd.php";
      }
      </script>
<?php
...
{
   ?><script>"ok".disabled = true;<script><?php
  echo "<script>alert('Order Placed')</script>";
  
  $date=date("Y-m-d");
  $pur_amt=0;
  $insert2 = mysqli_query($con,"INSERT INTO orderplaced(user_id,order_date,status,purchase_amount) VALUES ('$uid','$date',0,$pur_amt);");
  $odq="select max(id) as maxid from `orderplaced` where `user_id`='$uid'";
  ...
  $cart_data = json_decode($cookie_data, true);
    ...
    foreach($cart_data as $keys => $values)
    {
     ...
     $insert = mysqli_query($con,"INSERT INTO orderitems(orderid,qty,user_id,dish_name,price) VALUES ('$ordid','$qty','$uid','$dish',$price);");
